...
static std::string to32BitReg(const std::string& reg) {
    if (reg == "%rax" || reg == "rax") return "%eax";
    if (reg == "%rcx" || reg == "rcx") return "%ecx";
    if (reg == "%rdx" || reg == "rdx") return "%edx";
    if (reg == "%rbx" || reg == "rbx") return "%ebx";
    if (reg == "%rsi" || reg == "rsi") return "%esi";
    if (reg == "%rdi" || reg == "rdi") return "%edi";
    if (reg == "%r8" || reg == "r8") return "%r8d";
    if (reg == "%r9" || reg == "r9") return "%r9d";
    if (reg == "%r10" || reg == "r10") return "%r10d";
    if (reg == "%r11" || reg == "r11") return "%r11d";
    if (reg == "%r12" || reg == "r12") return "%r12d";
    if (reg == "%r13" || reg == "r13") return "%r13d";
    if (reg == "%r14" || reg == "r14") return "%r14d";
    if (reg == "%r15" || reg == "r15") return "%r15d";
    return reg;
}

static std::string to64BitReg(const std::string& reg) {
    if (reg == "%eax" || reg == "eax") return "%rax";
    if (reg == "%ecx" || reg == "ecx") return "%rcx";
    if (reg == "%edx" || reg == "edx") return "%rdx";
    if (reg == "%ebx" || reg == "ebx") return "%rbx";
    if (reg == "%esi" || reg == "esi") return "%rsi";
    if (reg == "%edi" || reg == "edi") return "%rdi";
    if (reg == "%r8d" || reg == "r8d") return "%r8";
    if (reg == "%r9d" || reg == "r9d") return "%r9";
    if (reg == "%r10d" || reg == "r10d") return "%r10";
    if (reg == "%r11d" || reg == "r11d") return "%r11";
    if (reg == "%r12d" || reg == "r12d") return "%r12";
    if (reg == "%r13d" || reg == "r13d") return "%r13";
    if (reg == "%r14d" || reg == "r14d") return "%r14";
    if (reg == "%r15d" || reg == "r15d") return "%r15";
    return reg;
}
...
static void emitMov(CodeGen& cg, std::ostream* os, const std::string& src, const std::string& dst, bool is32) {
    ...
    if (!cg.lastStoreOp.empty() && s == cg.lastStoreOp && (d == regRax || d == "%rax" || d == "%eax")) return;
