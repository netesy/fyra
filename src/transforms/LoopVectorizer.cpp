...
bool LoopVectorizer::performTransformation(ir::Function& func) {
    ...
    for (auto bbIt = func.getBasicBlocks().begin(); bbIt != func.getBasicBlocks().end(); ++bbIt) {
        ...
        if (reductionPhi && reductionPhi->getType() && reductionPhi->getType()->isInteger()) {
            ...
            ir::Value* incomingLatch = reductionPhi->getIncomingValueForBlock(latchBB);
            if (!incomingLatch && predication) incomingLatch = reductionPhi->getIncomingValueForBlock(predication->mergeBlock);
            auto* update = dynamic_cast<ir::Instruction*>(incomingLatch);
            if (initial && update && update->getOperands().size() == 2) {
                ...
                if (!term && predication && predication->mergePhi) {
                    if (lhs == predication->mergePhi) term = lhs;
                    else if (rhs == predication->mergePhi) term = rhs;
                }
        ...
        if (plan.isWideningReduction) {
            ...
            for (ir::BasicBlock* scalarBlock : scalarBlocks) for (auto& owner : scalarBlock->getInstructions()) {
                ...
                if (opc == ir::Instruction::Load || opc == ir::Instruction::Loaduw ||
                    ...
                    if (!no && plan.mainElemType) {
                        no = builder.createVSub(yes, yes); // 0 vector identity
                    }
                    if (mask && yes && no) {
                        ir::VectorInstruction* vSel = builder.createVSelect(mask, yes, no);
                        values[phi] = vSel;
                        if (!plan.reductions.empty() && rawPhiVSum) {
                            ir::VectorInstruction* vNextSum = builder.createVAdd(rawPhiVSum, vSel);
                            rawPhiVSum->addIncoming(vNextSum, vLoopBodyBB);
                        }
                    }
