...
void ABIAnalysis::insertABILowering(ir::Function& func) {
    ...
    for (auto& param : params) {
        ...
        if (param->getType()->isFloatingPoint()) { 
            if (floatIdx < floatArgRegs.size()) { 
                // In a real compiler, we'd map register names to indices
                // For now, we assume a direct mapping for simplicity
                param->setPhysicalRegister(floatIdx++); 
            } 
        } else { 
            if (intIdx < intArgRegs.size()) { 
                param->setPhysicalRegister(intIdx++); 
            } 
        }
    }
}
