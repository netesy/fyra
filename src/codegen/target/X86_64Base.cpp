...
void X86_64Base::emitBasicBlockStart(CodeGen& cg, ir::BasicBlock& bb) {
    ...
    if (auto* os = cg.getTextStream()) { *os << label << ":\n"; }
    else { CodeGen::SymbolInfo s; s.name = label; s.sectionName = ".text"; s.value = cg.getAssembler().getCodeSize(); s.type = 0; s.binding = 1; cg.addSymbol(s); }
}
...
void X86_64Base::emitLoadValue(CodeGen& cg, execgen::Assembler& as, ir::Value* v, uint8_t regIdx) {
    if (!v) {
        uint8_t rex = (regIdx >= 8) ? 0x49 : 0x48;
        as.emitByte(rex); as.emitByte(0xB8 + (regIdx & 7)); as.emitQWord(0);
        return;
    }
    if (auto* ci = dynamic_cast<ir::ConstantInt*>(v)) {
        uint8_t rex = (regIdx >= 8) ? 0x49 : 0x48;
        as.emitByte(rex); as.emitByte(0xB8 + (regIdx & 7)); as.emitQWord(ci->getValue());
    } else if (v->getName() == "__heap_ptr" || dynamic_cast<ir::GlobalVariable*>(v) || dynamic_cast<ir::GlobalValue*>(v)) {
        uint8_t rex = (regIdx >= 8) ? 0x4C : 0x48;
        as.emitByte(rex); as.emitByte(0x8B); as.emitByte(0x05 | ((regIdx & 7) << 3));
        uint64_t off = as.getCodeSize(); as.emitDWord(0);
        cg.addRelocation(CodeGen::RelocationInfo{off, "R_X86_64_PC32", -4, v->getName(), ".text"});
    } else {
        int32_t offset = cg.getStackOffset(v);
        uint8_t rex = (regIdx >= 8) ? 0x4C : 0x48;
        emitRegMem(as, rex, 0x8B, regIdx & 7, offset);
    }
}

void X86_64Base::emitStoreResult(CodeGen& cg, ir::Instruction& instr, uint8_t regIdx) {
    int32_t offset = cg.getStackOffset(&instr);
    uint8_t rex = (regIdx >= 8) ? 0x4C : 0x48;
    emitRegMem(cg.getAssembler(), rex, 0x89, regIdx & 7, offset);
}
...
bool X86_64Base::isReserved(const std::string& reg) const {
