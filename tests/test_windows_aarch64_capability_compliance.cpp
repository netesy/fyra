...
int main() {
    auto target_ptr = codegen::target::TargetResolver::resolve({::target::Arch::AArch64, ::target::OS::Windows}); auto& target = *target_ptr;
    ...
    std::cout << "Windows AArch64 capability compliance: " << pass << "/" << cases.size() << "\n";
