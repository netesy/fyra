...
int main() {
    std::string test_file = "tests/sub.fyra";
    ...
    std::cout << "Generated ASM for sub.fyra:\n" << generated_asm << std::endl;

    assert(generated_asm.find("sub") != std::string::npos);
    assert(generated_asm.find("10") != std::string::npos);
    assert(generated_asm.find("32") != std::string::npos);
